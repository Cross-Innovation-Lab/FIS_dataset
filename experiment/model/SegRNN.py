...
将 SegRNN 包装为当前实验统一接口：
- `forward(batch: dict) -> Tensor[B, 9]`
- 支持任务一 / 任务二
- 视觉/音频走 SegRNN 时序编码，文本采用现有 mean/cls 池化
"""
...
from models.SegRNN import Model as _SegRNNCore  # type: ignore[import]


@dataclass
class SegRNNFISConfig:
    video_dim: int = 235
    audio_dim: int = 768
    text_dim: int = 1024
    hidden_dim: int = 256
...
class _SegRNNSequenceEncoder(nn.Module):
    """单模态 SegRNN 序列编码器。"""

    def __init__(self, input_dim: int, cfg: SegRNNFISConfig):
        super().__init__()
        if cfg.seq_len % cfg.seg_len != 0:
            raise ValueError(f"SegRNN 需要 seq_len={cfg.seq_len} 可被 seg_len={cfg.seg_len} 整除")
        ...
        self.core = _SegRNNCore(core_cfg)
...
class SegRNNFIS(nn.Module):
    """SegRNN 基线，适配当前 FIS 多模态回归任务。"""

    def __init__(self, cfg: SegRNNFISConfig):
        super().__init__()
        self.cfg = cfg
        fusion_dim = 0

        if cfg.use_visual:
            self.visual_enc = _SegRNNSequenceEncoder(cfg.video_dim, cfg)
            fusion_dim += cfg.hidden_dim
        else:
            self.visual_enc = None

        if cfg.use_audio:
            self.audio_enc = _SegRNNSequenceEncoder(cfg.audio_dim, cfg)
            fusion_dim += cfg.hidden_dim
        else:
            self.audio_enc = None
