...
将 MSGNet 包装为当前实验统一接口：
- `forward(batch: dict) -> Tensor[B, 9]`
- 视觉/音频走 MSGNet 分类式时序编码
- 文本沿用现有实验中的池化逻辑
- 支持任务一 / 任务二
"""
...
from models.MSGNet import Model as _MSGNetCore  # type: ignore[import]


@dataclass
class MSGNetFISConfig:
    video_dim: int = 235
    audio_dim: int = 768
    text_dim: int = 1024
    hidden_dim: int = 256
...
class _MSGNetSequenceEncoder(nn.Module):
    """单模态 MSGNet 序列编码器。"""

    def __init__(self, input_dim: int, cfg: MSGNetFISConfig):
        super().__init__()
        if cfg.graph_nodes > cfg.d_model:
            raise ValueError(f"MSGNet 需要 graph_nodes={cfg.graph_nodes} 不大于 d_model={cfg.d_model}")
        ...
        core_cfg = SimpleNamespace(
            ...
            freq=cfg.freq,
            top_k=cfg.top_k,
            conv_channel=cfg.conv_channel,
            skip_channel=cfg.skip_channel,
            gcn_depth=cfg.gcn_depth,
            propalpha=cfg.propalpha,
            node_dim=cfg.node_dim,
            individual=cfg.individual,
            c_out=cfg.graph_nodes,
            num_class=cfg.hidden_dim,
        )
        self.core = _MSGNetCore(core_cfg)
...
class MSGNetFIS(nn.Module):
    """MSGNet 基线，适配当前 FIS 多模态回归任务。"""

    def __init__(self, cfg: MSGNetFISConfig):
        super().__init__()
        self.cfg = cfg
        fusion_dim = 0

        if cfg.use_visual:
            self.visual_enc = _MSGNetSequenceEncoder(cfg.video_dim, cfg)
            fusion_dim += cfg.hidden_dim
        else:
            self.visual_enc = None

        if cfg.use_audio:
            self.audio_enc = _MSGNetSequenceEncoder(cfg.audio_dim, cfg)
            fusion_dim += cfg.hidden_dim
        else:
            self.audio_enc = None
