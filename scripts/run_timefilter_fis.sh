...
# ============================================================================
#  TimeFilter-FIS 基线 训练 / 评估 启动脚本
#
#  用法:
#    训练任务一:  bash scripts/run_timefilter_fis.sh train task1
#    训练任务二:  bash scripts/run_timefilter_fis.sh train task2
#    后台训练:    bash scripts/run_timefilter_fis.sh train task1 --bg
#    评估:        bash scripts/run_timefilter_fis.sh eval  task1  <ckpt_path> [split]
#
...
case "${TASK}" in
  task1) CONFIG="${CONFIG_DIR}/timefilter_task1.json" ;;
  task2) CONFIG="${CONFIG_DIR}/timefilter_task2.json" ;;
  *)
    if [[ -f "${TASK}" ]]; then
      CONFIG="${TASK}"
    else
...
if [[ "${CMD}" == "train" ]]; then
  if [[ "${EXTRA}" == "--bg" ]]; then
    ...
    CONSOLE_LOG="${PROJECT_ROOT}/outs/timefilter_fis_${TIMESTAMP}_console.log"
  ...
  if [[ -z "${CKPT}" ]]; then
    ...
    echo "用法: bash scripts/run_timefilter_fis.sh eval task1 checkpoints/xxx/best.pt [split]"
    exit 1
  fi
